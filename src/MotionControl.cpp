...
void MotionControlNode::updateTwist(const geometry_msgs::msg::Pose &target_pose) {
    ...
    double dy = target_pose.position.y - current_pose_.pose.position.y;   //vzdialenost medzi akt a ciel

    double desired_heading = std::atan2(dy, dx);    //pozadovany smer ktorym chceme ist

    tf2::Quaternion q(
        current_pose_.pose.orientation.x,
        current_pose_.pose.orientation.y,
        current_pose_.pose.orientation.z,
        current_pose_.pose.orientation.w);    //aktualne udaje

    double roll, pitch, yaw;
    tf2::Matrix3x3(q).getRPY(roll, pitch, yaw);   

    double heading_error = desired_heading - yaw;             //pozadovany sme a akutalny smer
    while (heading_error > M_PI) heading_error -= 2*M_PI;
    while (heading_error < -M_PI) heading_error += 2*M_PI;    //normalizacia aby sa robot neotacal

    geometry_msgs::msg::Twist twist;
    twist.linear.x = 0.1;  // rachlost
    twist.angular.z = 1.2 * heading_error; // P riadenie smeru
...
{
    (void)uuid;  // Pokud UUID nepouivame
    if (!rclcpp::ok()) {
...
void MotionControlNode::execute() {
    ...
    while (rclcpp::ok() && goal_handle_ && goal_handle_->is_active()) {   // ak nie som v cieli
        ...
        if (distance < 0.1) {   //ak som blizko bodu, idem na dalsi
            ++target_idx;
            continue;
        }

        updateTwist(target_pose);  // Už nic nevrací
